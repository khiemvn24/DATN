...
export default function FindProduct(props) {
  ...
  const getLoaiSp = async () => {
    try {
      ...
      setListTTSp(res.data.filter((el) => el.name.toLowerCase().includes(props.name.toLowerCase())));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const ShowLoaiSP = funcSort(listTTSp).map((itemSP, indexSP) => {
    ...
    >
      <Link className="w-full h-full flex flex-col justify-start items-start text-underline-hover" to={`/product-detail/${itemSP.id}`}>
        <p
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start">
      ...
      <div className="w-full flex flex-col justify-center items-center text-underline-hover">
